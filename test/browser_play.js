...
async function main() {
  ...
  ok(await evaljs("Array.prototype.some.call(document.querySelectorAll('#action-panel .btn'), function(b){ return b.textContent.indexOf('🗺️') !== -1; }) && Array.prototype.some.call(document.querySelectorAll('#action-panel .btn'), function(b){ return b.textContent.indexOf('🏘️') !== -1; }) && Array.prototype.some.call(document.querySelectorAll('#action-panel .btn'), function(b){ return b.textContent.indexOf('🚶') !== -1; })"), '野外操作面板恢复地图/返回城镇/前往下个地点三个按钮');
  ok(await evaljs("(function(){ var labels=Array.prototype.map.call(document.querySelectorAll('#action-panel .btn'), function(b){ return b.textContent; }); var i1=labels.findIndex(function(t){return t.indexOf('地图')!==-1;}); var i2=labels.findIndex(function(t){return t.indexOf('返回城镇')!==-1;}); var i3=labels.findIndex(function(t){return t.indexOf('前往下个地点')!==-1;}); return i1>=0 && i2>i1 && i3>i2; })()"), '三个按钮顺序为 地图→返回城镇→前往下个地点');
  await evaljs("doMapAction('travel');");
  ok(await evaljs("document.querySelectorAll('#modal-root .travel-btn').length >= 1"), '前往下个地点弹窗可打开');
  await evaljs('closeModal();');
  await evaljs('closeModal();');
  await evaljs('doMapAction(\'travel\');');
  ...
  ok(await evaljs("Array.prototype.some.call(document.querySelectorAll('#modal-root .shop-row'), function(r){ return r.textContent.indexOf('重生药') !== -1 && r.textContent.indexOf('200000金') !== -1; })"), '8徽章商店上架重生药（20万金）');
  ...
  ok(await evaljs("Array.prototype.some.call(document.querySelectorAll('#modal-root .shop-row'), function(r){ return r.textContent.indexOf('重生药') !== -1 && r.textContent.indexOf('使用') !== -1; })"), '背包里可使用重生药');
  ...
  await evaljs("window.__remoteLastAction = null; remoteSubmit = function(action){ window.__remoteLastAction = action; REMOTE.lastView.battle.actions_submitted.A = true; REMOTE.switchPromptKey = ''; closeModal(); remoteRenderBattle(REMOTE.lastView); }; REMOTE.battleOpen = true; REMOTE.side = 'A'; REMOTE.roomId = 'room-switch-test'; REMOTE.lastView = { id: 'room-switch-test', you: 'A', players: { A: { name: '小智' }, B: { name: '小茂' } }, battle: { phase: 'switch', turn: 3, over: false, weather: null, actions_submitted: { A: false, B: false }, pending_switch: ['A'], actives: { A: { uid: 101, species: 25, name: '皮卡丘', level: 50, hp: 0, max: 120, status: '麻痹', held: null, moves: [{ id: 'thunderbolt', pp: 10 }] }, B: { uid: 201, species: 7, name: '杰尼龟', level: 50, hp: 80, max: 120, status: null, held: null, moves: [{ id: 'water_gun', pp: 20 }] } }, parties: { A: [{ uid: 101, species: 25, name: '皮卡丘', level: 50, hp: 0, max: 120, status: '麻痹', held: null, moves: [] }, { uid: 102, species: 6, name: '喷火龙', level: 50, hp: 150, max: 150, status: null, held: '不灭之种', moves: [] }, { uid: 103, species: 9, name: '水箭龟', level: 50, hp: 132, max: 160, status: null, held: null, moves: [] }], B: [{ uid: 201, species: 7, name: '杰尼龟', level: 50, hp: 80, max: 120, status: null, held: null, moves: [] }] }, items: { A: {}, B: {} } }, events: [] }; remoteRenderView(REMOTE.lastView);");
  ok(await evaljs("document.querySelector('#modal-root .modal-header').textContent.indexOf('选择下一只出战宝可梦') !== -1"), '联机待换人时自动弹出选首发弹窗');
  ok(await evaljs("document.querySelector('#rb-actions').textContent.indexOf('请在弹框中选择下一只出战宝可梦') !== -1 && document.querySelector('#rb-actions').textContent.indexOf('喷火龙') === -1"), '联机主操作区不再直接展示候选宝可梦按钮');
  await evaljs("REMOTE.lastView.battle.phase = 'action'; REMOTE.lastView.battle.pending_switch = []; REMOTE.lastView.battle.actions_submitted = { A: false, B: false }; REMOTE.lastView.events = [{ seq: 1, kind: 'a', text: '小智 使用了【电击】！' }, { seq: 2, kind: 'b', text: '杰尼龟 造成了 20 点伤害！' }]; REMOTE.seen = 0; remoteRenderBattle(REMOTE.lastView);");
  ok(await evaljs("document.querySelector('#rb-log').textContent.indexOf('小智(皮卡丘) 使用了【电击】！') !== -1 && document.querySelector('#rb-log').textContent.indexOf('小茂(杰尼龟) 造成了 20 点伤害！') !== -1"), 'PvP 日志显示玩家(宝可梦)格式');
  await evaljs("Array.prototype.slice.call(document.querySelectorAll('#modal-root .modal-body .btn')).filter(function(b){ return b.textContent.indexOf('喷火龙') !== -1; })[0].click();");
  ok(await evaljs("window.__remoteLastAction && window.__remoteLastAction.type === 'switch' && window.__remoteLastAction.index === 1"), '联机换人弹窗选择后提交 switch 指令');
  ok(await evaljs("document.querySelector('#rb-actions').textContent.indexOf('已提交换人') !== -1 && document.querySelector('#modal-root').textContent.indexOf('选择下一只出战宝可梦') === -1"), '联机换人提交后关闭弹窗并显示等待提示');
